...
本文先仅根据 [`problem_official.md`](../spec/problem_official.md)、[`cann_api_reference.md`](cann_api_reference.md) 形成独立设计并落盘，然后阅读 [`plan.md`](plan.md)、[`memory.md`](../history/memory.md)、[`kernel.asc`](../../kernel.asc)、测试和工程文件，最后整合。历史实验用于修正假设，不作为不可改变的架构约束。下文分别标明数学结论、历史观测、设计候选和待验证接口，未把设计推演写成 NPU 实测结果。
...
3. **旧脚本绑定具体实现。** [`tests/reference_test.py`](../../tests/reference_test.py) 包含函数名、常量和源码字符串断言；适合保护旧版本，不能把“违反旧字符串断言”直接当作新算法错误。其 NumPy 模型也不验证实际队列、GM 可见性、异步 C stride 或 NPU 资源。
...
只交付比赛要求的 [`kernel.asc`](../../kernel.asc) 时，也可在文件内分层：
